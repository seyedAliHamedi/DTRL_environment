...
class Agent:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.learning_device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
            devices = Database.get_all_devices()
            cls._instance.devices = devices
            cls._instance.core = CoreScheduler(devices)
            cls._instance.device = DeviceScheduler(devices)
            net = ValueNetwork(input_size=4 * len(devices) + 5).to(cls._instance.learning_device)
    ...
    def schedule(self, task_queue, job_id):
        ...
        input_state = torch.tensor(input_state, dtype=torch.float32).to(self.learning_device)
        ...
        if selected_device['type'] != "cloud":
            ...
            sub_state = torch.tensor(sub_state,dtype=torch.float32).to(self.learning_device)
        ...
        if len(task_queue) <= 0:
            ...
            next_input_state = torch.tensor(temp_features, dtype=torch.float32).to(self.learning_device)
            ...
            next_input_state = torch.tensor(next_input_state, dtype=torch.float32).to(self.learning_device)
            next_value = self.value_net(next_input_state)
        ...
        option_loss = -option_dist.log_prob(option.clone().detach()) * advantage
        if action_dist is not None and action is not None:
            actor_loss = -action_dist.log_prob(action.clone().detach()) * advantage
        else:
            actor_loss = torch.tensor(0.0)  # Placeholder for cases without action_dist/action
    ...
    def compute_advantages(self, rewards, values, gamma=0.99, normalize=True):
        ...
        for t in reversed(range(len(rewards))):
            td_error = (rewards[t] + gamma * (0 if t == len(rewards) - 1 else values[t + 1]) - values[t])
