...
class Agent:
    ...
    def schedule(self, assigned_task, job_id):
        ...
        job_state, pe_state = self.env.state.get()
        ...
        reward, fail_flag, energy, time = self.env.state.apply_action(selected_device_index,
                                                                      selected_core_index, dvfs[0], dvfs[1],
                                                                      current_task_id)

        if self.display:
            print(f"Agent|task={current_task_id} | fail={fail_flag}")
        ...
        job_state = self.env.state.get_job(job_id)
        if job_state and (job_state['task_count'] == len(job_state['runningTasks']) + len(job_state['finishedTasks'])):
            self.env.state.jobs_done += 1
